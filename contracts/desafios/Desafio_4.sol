...
contract Desafio_4 {
    struct Subasta{
        ...
        uint256 inicio;
        uint256 fin;
        uint256 apuestaMayor;
        address apostadorMayor;
        mapping(address => uint256) offers;
    }

    mapping (bytes32 => Subasta) public subastas;
    bytes32[] public listaSubastas;
    ...
    function creaSubasta(uint256 _startTime, uint256 _endTime) public payable {
        ...
        subasta.creador = msg.sender;
        subasta.inicio = _startTime;
        subasta.fin = _endTime;

        listaSubastas.push(_auctionId);
        indexes[_auctionId] = counter;
        counter++;
    ...
    function proponerOferta(bytes32 _auctionId) public payable {
        ...
        if (indexes[_auctionId] == 0 && listaSubastas.length == 0 || listaSubastas[indexes[_auctionId]] != _auctionId){
            revert SubastaInexistente();
        }

        if (msg.value < subasta.apuestaMayor){
            revert OfertaInvalida();
        }

        if (block.timestamp < subasta.inicio || block.timestamp > subasta.fin){
            revert FueraDeTiempo();
        }

        if (subasta.fin - block.timestamp <= 5 minutes) {
            subasta.fin += 5 minutes;
        }

        subasta.apuestaMayor = msg.value;
        subasta.apostadorMayor = msg.sender;
        subasta.offers[msg.sender] += msg.value; 
        subasta.recolectado += msg.value;
    ...
    function finalizarSubasta(bytes32 _auctionId) public {
        ...
        if (block.timestamp < subasta.fin){
            revert SubastaEnMarcha();
        }

        if (counter > 0) {
            uint256 ix = indexes[_auctionId];
            delete indexes[_auctionId];
            bytes32 lastEl = listaSubastas[listaSubastas.length - 1];
            listaSubastas[ix] = lastEl;
            listaSubastas.pop();
            indexes[lastEl] = ix;
            counter--;
        }
        
        subasta.offers[subasta.apostadorMayor] += 1 ether;
        emit SubastaFinalizada(subasta.apostadorMayor, subasta.apuestaMayor);
    }
    ...
    function recuperarOferta(bytes32 _auctionId) public {
        ...
        payable(msg.sender).transfer(subasta.offers[msg.sender]);
    }
